...
export async function POST(request: Request) {
  try {
    ...
    const { nombre, tipo, orden, color } = body
    ...
    const nuevo = await createEstadoServicio(
      nombre.trim(),
      tipo,
      typeof orden === "number" ? orden : undefined,
...
export async function PUT(request: Request) {
  try {
    ...
    const body = await request.json()
    const { id, nombre, tipo, orden, visible, color } = body
